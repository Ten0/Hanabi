...
public class CardKnowlege {
	...
	public TreeSet<Color> getPossibleColors() {
		...
		while(it.hasNext()) {
			...
			for(Clue clue : clues) if (clue instanceof ColorClue) {
			...
			if(!remove) {
				for(Clue clue : negativeClues) if (clue instanceof ColorClue) {
					ColorClue cClue = (ColorClue)clue;
					if(cClue.matches(color)) {
						remove = true;
						break;
	...
	public TreeSet<Integer> getPossibleNumbers() {
		...
		while(it.hasNext()) {
			int number = it.next();
			boolean remove = false;
			for(Clue clue : clues) if (clue instanceof NumberClue) {
			...
			if(!remove) {
				for(Clue clue : negativeClues) if (clue instanceof NumberClue) {
					NumberClue nClue = (NumberClue)clue;
					if(nClue.getNumber() == number) {
						remove = true;
						break;
